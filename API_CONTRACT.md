...
**الجدول في قاعدة البيانات:** `Tables`
**الغرض:** تخزين معلومات كل طاولة في المطعم، والأهم من ذلك، ربط رقم الطاولة البسيط (للعرض) بمعرف فريد وآمن (UUID) يستخدم في الروابط.

| الحقل (Field)     | النوع (Type) | الوصف                                        | ملاحظات                               |
| ------------------ | ------------- | -------------------------------------------- | -------------------------------------- |
| `id`               | `integer`     | رقم الطاولة للعرض (مثال: 1, 2, 5)            | Primary Key, Auto-increment          |
| `uuid`             | `string`      | معرف فريد عالمي (UUID) للطاولة               | مطلوب، فريد، يستخدم في روابط QR Code |
| `is_active`        | `boolean`     | هل الطاولة حالياً في الخدمة؟                 | افتراضي: `true`                       |
| `created_at`       | `timestamp`   | تاريخ إضافة الطاولة                          | يضاف تلقائياً                          |
...
| --------------------------- | ------------------------- | ----------------------------------------------- | ----------------------------------- |
| `id`                        | `string`                  | معرف فريد (UUID)                                | Primary Key                         |
| `table_id`                  | `integer`                 | رقم الطاولة المرتبط بها الطلب (Foreign Key)     | يربط مع جدول `Tables`              |
...
### د. تفاصيل الطلب (OrderItem) - (جدول وسيط)
...
*   **`GET /`**: جلب جميع أصناف القائمة لعرضها في `/menu-management` و `/menu/[uuid]`.
...
*   **`POST /`**: إنشاء طلب جديد (يتم من واجهة الزبون `/menu/[uuid]`).
    *   **Body**: `{ table_uuid, items: [{ menu_item_id, quantity }] }`
